...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            # if all of the precondition nodes of the proposed node are in the
            # previous S level, add the proposed node to the current A level
            if proposed_node.prenodes.issubset(self.s_levels[level]):
                a_nodes.append(proposed_node)
                for s_node in self.s_levels[level]: 
                    # 2. connect the nodes to the previous S literal level
                    proposed_node.parents.add(s_node)
                    s_node.children.add(proposed_node)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # if any of the effect_add of one action is in the effect_rem of another, flag as inconsistent
        return (any(effect in node_a1.action.effect_add for effect in node_a2.action.effect_rem) or
            any(effect in node_a2.action.effect_add for effect in node_a1.action.effect_rem))
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # if any of the effect_add of one action is in the precond_neg of another, 
        # or if any of the effect_rem of one action is in the precond_pos of another,  
        # flag as inconsistent
        return (any(precond in node_a1.action.effect_add for precond in node_a2.action.precond_neg) or
            any(precond in node_a1.action.effect_rem for precond in node_a2.action.precond_pos) or
            any(precond in node_a2.action.effect_add for precond in node_a1.action.precond_neg) or
            any(precond in node_a2.action.effect_rem for precond in node_a1.action.precond_pos))
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # note to self: This part requires precondition of one action to be mutex 
        # with precondition of the other. We cannot simply use precond_pos of 
        # one action to be in the precond_neg of another, as it is only one type
        # of mutex. We have to account for the other case of mutex where the
        # actions leading to one state is mutex with the actions leading to the
        # other state
        
        
        # To account for both types of mutex, we just use the is_mutex method
        return any(state_1.is_mutex(state_2) for state_1 in node_a1.parents for state_2 in node_a2.parents)
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        """
        # return True if all actions from the parents of one state is mutex with the other
    ...
    def h_levelsum(self) -> int:
        ...
        for clause in self.problem.goal:
            # determines if we found the lowest level that contains the clause
            clause_found = False 
            level = 0
            while not clause_found:
                # for each state, see if it meets goal
                for s_node in self.s_levels[level]:
